...
const handleScrapedTournaments = (message: any, sender: any, sendResponse: any): void => {
...
const createEvent = (tournament: any, token: string): Promise<any> => {
  ...
  const endDateTime = new Date(new Date(tournament.start).getTime() + 60 * 60 * 1000).toISOString();
...
const saveTournamentsToGoogleCalendar = async (tournaments: any[]): Promise<void> => {
  try {
    const token = await getToken();
    await Promise.all(tournaments.map((tournament) => createEvent(tournament, token)));

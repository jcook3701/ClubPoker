---
title: README
---
[**ClubPoker**](../README.md)

***

# types

## Enumerations

- [ErrorCode](enumerations/ErrorCode.md)
- [ThemeMode](enumerations/ThemeMode.md)
- [WarningCode](enumerations/WarningCode.md)

## Interfaces

- [AppSettings](interfaces/AppSettings.md)
- [Calendar](interfaces/Calendar.md)
- [CalendarEvent](interfaces/CalendarEvent.md)
- [CalendarEvents](interfaces/CalendarEvents.md)
- [Club](interfaces/Club.md)
- [ErrorMessage](interfaces/ErrorMessage.md)
- [Filter](interfaces/Filter.md)
- [FilterItem](interfaces/FilterItem.md)
- [Timezone](interfaces/Timezone.md)
- [Tournament](interfaces/Tournament.md)
- [Tournaments](interfaces/Tournaments.md)
- [ViewMode](interfaces/ViewMode.md)
- [WarningMessage](interfaces/WarningMessage.md)

## Type Aliases

- [ClubType](type-aliases/ClubType.md)
- [FiltersState](type-aliases/FiltersState.md)
- [FilterState](type-aliases/FilterState.md)

## Variables

- [ClubTypes](variables/ClubTypes.md)
- [ClubTypeValues](variables/ClubTypeValues.md)

## Functions

- [boolToTheme](functions/boolToTheme.md)
- [formatErrorMessage](functions/formatErrorMessage.md)
- [formatWarningMessage](functions/formatWarningMessage.md)
- [themeToBool](functions/themeToBool.md)
